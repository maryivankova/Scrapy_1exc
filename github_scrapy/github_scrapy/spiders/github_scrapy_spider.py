...
class GithubScrapySpider(CrawlSpider):
    ...
    def parse_rep(self, response):
        ...
        items = GithubScrapyItem()

        change_log = self.parse_rel(response)
           
        commit_tuple = [
                response.css('div.css-truncate a::text')[0:2].getall(),           
                response.css('a.Link--secondary relative-time::attr(datetime)').get(),
            ]
        ...
        release_tuple = [
            ...
            response.css('div.text-small relative-time::attr(datetime)').getall(),
            response.css('div.mt-3 a::attr(href)').extract_first(),
            change_log
        ...
        yield{
            ...
            'watching_num':response.css('a.Link--muted strong::text')[1:2].get(),
            'commits_num':response.css('span.d-none strong::text').get(),
            'commit_last':commit_str,         
            'releases_num':response.css('a.Link--primary span.Counter::text').get(),
